...
namespace MSLib.Editor.Tools.FileFocus
{
    public sealed class EditorWindowFileFocusTask : IEditorWindowTask
    {
                    ...
                    foreach (var pathInfo in editorFilePathsAsset.GameObjectPathList)
                    {
                        using (new GUILayout.HorizontalScope())
                        {
                            ...
                            if (pathInfo.target != null)
                                EditorGUILayout.ObjectField(pathInfo.target, typeof(Object), true,
        ...
        public void Execution()
        {
            ...
            var instance = projectBrowserType.GetField("s_LastInteractedProjectBrowser",
                System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public).GetValue(null);

            var methodInfo = projectBrowserType.GetMethod("ShowFolderContents",
